...
class AdaptiveRotationEngine:
    ...
    def __init__(
        ...
        print(">>> USING QQQ MODIFIED ENGINE <<<")
        """
    ...
    def _qqq_rolling_prefilter(
        ...
        regime_result=None,
        skip_weeks: int = 4,
        lookback_6m_weeks: int = 26,
        lookback_12m_weeks: int = 52,
        vol_lookback_weeks: int = 13,
    ) -> List[str]:
        """
        Assignment 2 enhanced QQQ rolling filter.

        Covers four focus areas:
        1. QQQ rolling logic: 6M/12M momentum, skipping recent period.
        2. Alpha signal: combined intermediate/long-term momentum.
        3. Regime detection: stricter selection when regime is not risk_on.
        4. Risk control: penalize or remove high-volatility names.
        """

        available_symbols = [
            s for s in symbols
            if s in prices and len(prices[s].dropna()) > 0
        ]
        ...
        price_df = pd.DataFrame({
            s: prices[s] for s in available_symbols
        }).sort_index()

        price_df = price_df.loc[price_df.index <= as_of_date]
        price_df = price_df.ffill().dropna(axis=1, how="all")

        min_len = lookback_12m_weeks + skip_weeks + 2
        if len(price_df) < min_len:
            return available_symbols

        # ------------------------------------------------------------
        # Regime-aware strictness
        # ------------------------------------------------------------
        regime_state = "unknown"
        if regime_result is not None:
            regime_state = str(getattr(regime_result, "effective_state", "unknown"))

        if "risk_on" in regime_state:
            top_k = 4
            max_weekly_vol = 0.09
        elif "neutral" in regime_state:
            top_k = 3
            max_weekly_vol = 0.07
        else:
            top_k = 2
        ...
        # Skip most recent weeks to avoid short-term reversal
        past = price_df.iloc[:-skip_weeks] if skip_weeks > 0 else price_df

        scores = {}

        for symbol in past.columns:
            s = past[symbol].dropna()

            if len(s) < lookback_12m_weeks:
                continue

            # --------------------------------------------------------
            # QQQ rolling / alpha signal
            # --------------------------------------------------------
            mom6 = s.iloc[-1] / s.iloc[-lookback_6m_weeks] - 1.0
            mom12 = s.iloc[-1] / s.iloc[-lookback_12m_weeks] - 1.0

            raw_momentum = 0.5 * mom6 + 0.5 * mom12

            # --------------------------------------------------------
            # Risk control: penalize high volatility
            # --------------------------------------------------------
            returns = s.pct_change().dropna()
            recent_vol = returns.tail(vol_lookback_weeks).std()

            if pd.isna(recent_vol) or recent_vol <= 0:
                continue

            # Hard volatility filter
            if recent_vol > max_weekly_vol:
                continue

            # Risk-adjusted alpha score
            scores[symbol] = raw_momentum / recent_vol
        ...
        print(
            f"[Enhanced QQQ Filter] regime={regime_state}, "
            f"top_k={top_k}, selected={selected}"
        )
    ...
    def _rank_assets_in_groups(
        ...
        for group_name in active_groups:
            ...
            # Original symbols in this asset group
            members = self.config.get_group_symbols(group_name)

            # Assignment 2 modification:
            # Apply Assignment 1 enhanced rolling selection logic to ALL asset groups
            original_members = members.copy()
            
            members = self._qqq_rolling_prefilter(
                prices=prices,
                symbols=members,
                as_of_date=as_of_date,
                regime_result=regime_result
            ...
            print(
                f"[Enhanced Rolling Filter - ALL GROUPS] {group_name}: "
                f"{original_members} -> {members}"
            )
            ...
            group_members_dict[group_name] = members
